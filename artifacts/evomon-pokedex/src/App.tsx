...
type SkillElementStyle = 'dark' | 'steel' | 'psychic' | 'light' | 'dragon' | 'bug' | 'poison' | 'grass' | 'electric' | 'water' | 'fighting';
...
function skillElementStyleFor(element: string): SkillElementStyle | null {
  ...
  if (normalized.includes('fighting')) return 'fighting';
  return null;
}
                  ...
                  <div className="skill-results" role="listbox" aria-label="Skill results" data-testid="skill-results">
                    ...
                    {filtered.map(({ key, move, displayName, ultimateRange, learners }) => {
                      ...
                      return <button type="button" role="option" aria-selected={selected?.key === key} aria-label={`${displayName}, ${move.element}, ${resultLevel}, ${learners.length} Mon`} className={`skill-result ${elementStyle ? `skill-result-${elementStyle}` : 'skill-result-default'} ${selected?.key === key ? 'selected' : ''}`} onClick={() => setSelectedKey(key)} key={key} data-testid={`skill-result-${displayName.toLowerCase().replaceAll(/[^a-z0-9]+/g, '-')}`}>
                        <span className="skill-result-icon" aria-hidden="true"><ElementIcon size={29} strokeWidth={2.5} fill={elementStyle === 'light' || elementStyle === 'dragon' || elementStyle === 'bug' || elementStyle === 'poison' || elementStyle === 'grass' || elementStyle === 'electric' || elementStyle === 'water' || elementStyle === 'fighting' ? 'currentColor' : undefined} /></span>
    ...
    <Shell>
      <main className="detail-main">
        ...
        <section className="detail-device">
          ...
          <section className="evolution-section"><div className="section-kicker"><span>02</span><h2 className="font-display">Evolution line</h2></div>{line.length ? <div className="evolution-line">{line.map((item, index) => { const evolutionEntry = catalog.find((candidate) => candidate.name === item); return <div className={`evolution-node ${item === entry.name ? 'current' : ''}`} key={`${item}-${index}`}><span>{String(index + 1).padStart(2, '0')}</span><b>{evolutionEntry ? <Link href={`/evomon/${encodeURIComponent(idFor(evolutionEntry))}`} data-testid={`link-evolution-${idFor(evolutionEntry)}`}>{item}</Link> : item}</b>{index < line.length - 1 && <ArrowRight size={15} />}</div>; })}</div> : <div className="missing-panel">Evolution line not recorded in the current wiki notes.</div>}</section>
